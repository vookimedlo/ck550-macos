...
extension CK550Command {
    ...
    private struct OffEffectCrosshairSpeedBytes {
        static func bytes(speed: OffEffectCrosshairSpeed) -> [uint8] {
            switch speed {
            case .Lowest:
                return [0x17, 0x01]
            case .Lower:
    ...
    enum OffEffectCrosshairSpeed: uint8 {
    ...
    static var setOffEffectCrosshairUNKNOWN_BEFORE_PACKETS: [uint8] {
        get {
    ...
    static func setOffEffectCrosshair(speed: OffEffectCrosshairSpeed, background: RGBColor, key: RGBColor) -> [[uint8]] {
        var result: [[uint8]] = [
        ...
        let bytes = OffEffectCrosshairSpeedBytes.bytes(speed: speed)
        result[0][36] = bytes[0]
        result[0][46] = bytes[1]
