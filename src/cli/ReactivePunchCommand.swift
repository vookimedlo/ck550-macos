...
public struct ReactivePunchCommand: CommandProtocol {
    ...
    public func run(_ options: Options) -> Result<(), CLIError> {
        ...
        typealias CK550Speed = CK550Command.OffEffectOverride.ReactivePunch.Speed
        ...
        cli.onOpen {
            ...
            if result {
                cli.setOffEffectReactivePunch(background: createRGBColor(options.backgroundColor)!, key: createRGBColor(options.color), speed: speed)
            }
