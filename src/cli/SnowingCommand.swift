...
public struct SnowingCommand: CommandProtocol {
    ...
    public func run(_ options: Options) -> Result<(), CLIError> {
        ...
        typealias CK550Speed = CK550Command.OffEffectOverride.Snowing.Speed
        ...
        cli.onOpen {
            ...
            if result {
                cli.setOffEffectSnowing(background: createRGBColor(options.backgroundColor)!, key: createRGBColor(options.color)!, speed: speed)
            }
