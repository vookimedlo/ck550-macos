...
public struct WaterRippleCommand: CommandProtocol {
    ...
    public func run(_ options: Options) -> Result<(), CLIError> {
        ...
        typealias CK550Speed = CK550Command.OffEffectOverride.WaterRipple.Speed
        ...
        cli.onOpen {
            ...
            if result {
                cli.setOffEffectWaterRipple(background: createRGBColor(options.backgroundColor)!, key: createRGBColor(options.color), speed: speed)
            }
