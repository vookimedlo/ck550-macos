...
public struct CrosshairCommand: CommandProtocol {
    ...
    public func run(_ options: Options) -> Result<(), CLIError> {
        ...
        typealias CK550Speed = CK550Command.OffEffectOverride.Crosshair.Speed
        ...
        cli.onOpen {
            ...
            if result {
                cli.setOffEffectCrosshair(background: createRGBColor(options.backgroundColor)!, key: createRGBColor(options.color)!, speed: speed)
            }
