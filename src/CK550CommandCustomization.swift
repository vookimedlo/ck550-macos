...
extension CK550Command {
    static func offEffectCustomizationPacket(packetId: Int) -> [uint8] {
        switch packetId {
        case 0:
            return newCommand(request: [0x56, 0x83, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x80, 0x01, 0x00, 0xC1, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF])
        default:
            var command = newCommand(request: [0x56, 0x83, 0x00])
    ...
    static var setOffEffectCustomizationUNKNOWN_AFTER_PACKETS: [uint8] {
        get {
            return newCommand(request: [0x40, 0x63])
        }
    }
}
...
class CK550OffEffectCustomizationLayoutUS: CK550OffEffectCustomizationLayout {
    ...
    required init() {
        ...
        mapping[Key.Apostrophe.rawValue] = CK550OffEffectCustomizationKeyData(packet: 4, offset: 0x09)
